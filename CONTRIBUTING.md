...
to the **framedf** project!

This document provides guidelines for contributing to framedf, its codebase and documentation.  
...
Clone the framedf repository:

```bash
git clone https://github.com/gcol33/framedf.git
cd framedf
```
...
framedf is an R package that uses C++ code via Rcpp.
...
framedf uses **testthat** for testing.  
...
testthat::test_file("tests/testthat/test-framedf.R")
```
...
└── framedf.Rcheck/      <- Artifacts from local package checks (ignored)
```
...
By contributing to framedf, you agree that your code is released under the same license as the package.
